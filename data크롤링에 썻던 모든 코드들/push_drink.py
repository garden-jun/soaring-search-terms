...
#Drink TABLE 에 업데이트
def insertInto(cursor,drinkName,temparture,caffeine,flavor):
    insertCommand = """ INSERT INTO Drink(drinkName,temparture,caffeine,flavor)
        values ("{}", "{}", "{}", "{}")""".format(drinkName,temparture,caffeine,flavor)
    lastInsertId = -1
...
for i in range(len(dessert_list)):
    if dessert_df.iloc[i]['drink'] == 'o':
        ...
        lastInsertId = insertInto(cursor,dessert_df.iloc[i]['dessertName'],dessert_df.iloc[i]['temperature'],dessert_df.iloc[i]['caffeine'],dessert_df.iloc[i]['flavor'])
